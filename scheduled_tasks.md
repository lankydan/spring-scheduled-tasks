Do you need to run a process everyday at the exact same time like an alarm? Spring's scheduled tasks are for you. Allowing you to annotate a method with `@Scheduled` causing it to run at the specific time or interval that is denoted inside it. In this post we will look at setting up a project that can use scheduled tasks as well as how to use the different methods for defining when they execute.
...
- `W` represents the nearest weekday of the month. For example if `15L` will trigger on 15th day of the month if it is a weekday, otherwise it will run on the closest weekday. This value cannot be used in a list of day values.
